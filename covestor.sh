...
for page in `seq 2 3`
do
    	curl http://covestor.com/search/page/$page  |egrep -o "http://covestor.com/[a-zA-Z-]+/[a-zA-Z-]+" |grep -v "/search/"|sort |uniq >> tmp0
done
...
do
	...
	cat tmp |egrep 'labelCol">\w+ [0-9]+, [0-9]+<' |cut -d'>' -f2 |cut -d'<' -f1 |while read date; do date -d "$date" +%m/%d/%Y; done |cat -n > tmp1
 	cat tmp |egrep -o -i '>Buy<|>Sell<|>Cover<|>Sell short<' |sed -e 's/<//g' -e 's/>//g' -e 's/Sell short/Short/g' |cat -n > tmp2
 	cat tmp |egrep '<td> <a href="http://stocks.covestor.com/\w+">[A-Z0-9 ]+<' |cut -d'>' -f3 |cut -d'<' -f1 |cat -n |awk '{print $1,$2}' > tmp3
...
\rm -f tmp tmp0 tmp1 tmp2 tmp3 tmp4
